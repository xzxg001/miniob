...
RC Db::init(const char *name, const char *dbpath, const char *trx_kit_name, const char *log_handler_name)
{
  ...
  const char      *double_write_buffer_filename  = "dblwr.db";  // 双写缓冲文件名
  ...
  filesystem::path clog_path       = filesystem::path(dbpath) / "clog";  // 日志路径
...
RC Db::create_table(const char *table_name, span<const AttrInfoSqlNode> attributes, const StorageFormat storage_format)
{
  ...
  if (rc != RC::SUCCESS) {
    ...
    delete table;  // 创建失败时释放内存
    return rc;
  }

  opened_tables_[table_name] = table;  // 记录打开的表
...
RC Db::drop_table(const char *name)
{
  ...
  string  table_file_path = table_meta_file(path_.c_str(), name);
  rc = table->drop(table_file_path.c_str());
  opened_tables_.erase(string(name));  // 从打开的表中移除
  return rc;
}
...
Table *Db::find_table(const char *table_name) const
{
  ...
  if (iter != opened_tables_.end()) {
    return iter->second;  // 找到表返回
  }
  return nullptr;  // 未找到表返回nullptr
}
...
RC Db::open_all_tables()
{
  ...
  for (const string &filename : table_meta_files) {
    ...
    if (opened_tables_.count(table->name()) != 0) {
      ...
      delete table;  // 释放内存以防止use-after-free错误
      return RC::INTERNAL;
    }

    if (table->table_id() >= next_table_id_) {
      next_table_id_ = table->table_id() + 1;  // 更新下一个表ID
    }
    opened_tables_[table->name()] = table;  // 记录打开的表
...
RC Db::sync()
{
  ...
  rc                = dblwr_buffer->flush_page();  // 刷新双写缓冲区
  LOG_INFO("double write buffer flush pages ret=%s", strrc(rc));

  // 在sync期间，不允许有未完成的事务，也不允许开启新的事务。
  LSN current_lsn = log_handler_->current_lsn();  // 当前日志序列号
  ...
  check_point_lsn_ = current_lsn;  // 更新检查点日志序列号
  rc               = flush_meta();  // 刷新元数据到磁盘
  if (OB_FAIL(rc)) {
...
RC Db::recover()
{
  ...
  rc = log_handler_->start();  // 启动日志处理器
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to start log handler. rc=%s", strrc(rc));
    return rc;
  }

  rc = log_replayer.on_done();  // 日志重放完成
  if (OB_FAIL(rc)) {
...
RC Db::init_meta()
{
  ...
  if (!filesystem::exists(db_meta_file_path)) {
    check_point_lsn_ = 0;  // 如果元数据文件不存在，检查点日志序列号设为0
    LOG_INFO("Db meta file not exist. db=%s, file=%s", name_.c_str(), db_meta_file_path.c_str());
    return RC::SUCCESS;
  }

  RC  rc = RC::SUCCESS;
  int fd = open(db_meta_file_path.c_str(), O_RDONLY);  // 以只读方式打开元数据文件
  if (fd < 0) {
  ...
  char buffer[1024];
  int  n = read(fd, buffer, sizeof(buffer));  // 读取元数据
  if (n < 0) {
...
RC Db::flush_meta()
{
  ...
  RC  rc = RC::SUCCESS;
  int fd = open(temp_meta_file_path.c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0644);  // 创建临时文件
  if (fd < 0) {
    LOG_ERROR("Failed to open db meta file. db=%s, file=%s, errno=%s", 
  ...
  string buffer = std::to_string(check_point_lsn_);  // 将检查点日志序列号转为字符串
  int    n      = write(fd, buffer.c_str(), buffer.size());  // 写入临时文件
  if (n < 0) {
    ...
    error_code ec;
    filesystem::rename(temp_meta_file_path, meta_file_path, ec);  // 重命名临时文件为正式文件
    if (ec) {
...
{
  auto dblwr_buffer = static_cast<DiskDoubleWriteBuffer *>(buffer_pool_manager_->get_dblwr_buffer());
  RC   rc           = dblwr_buffer->recover();  // 恢复双写缓冲区
  if (OB_FAIL(rc)) {
