...
public:
  ...
  virtual ~DataType() = default;  // 虚析构函数，允许派生类的析构

  // 根据属性类型返回相应的数据类型实例
  inline static DataType *type_instance(AttrType attr_type)
  {
    return type_instances_.at(static_cast<int>(attr_type)).get();  // 返回指定类型的实例
  }
