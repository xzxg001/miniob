...
{
public:
  BufferedWriter(int fd);// 使用文件描述符fd和默认缓存大小构造BufferedWriter对象
  BufferedWriter(int fd, int32_t size);// 使用文件描述符fd和指定的缓存大小size构造BufferedWriter对象
  ~BufferedWriter();// 析构函数，负责关闭缓存并释放资源
