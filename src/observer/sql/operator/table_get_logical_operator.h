...
#include "sql/operator/logical_operator.h"  // 引入逻辑算子基类
#include "storage/field/field.h"            // 引入字段相关类
#include "common/types.h"                   // 引入基本类型定义
...
public:
  // 构造函数，接受表对象和读写模式
  TableGetLogicalOperator(Table *table, ReadWriteMode mode) : table_(table), mode_(mode) {}

  virtual ~TableGetLogicalOperator() = default;  // 默认析构函数
  ...
  {
    predicates_ = std::move(exprs);  // 移动传入的过滤条件表达式
  }
